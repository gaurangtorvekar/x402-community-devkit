...
export class X402DevKit {
  ...
  async getWallet(seed?: string): Promise<WalletWithPrivateKey> {
    if (!this.walletPromise) {
      this.walletPromise = this.initializeWallet(seed);
    }

    return this.walletPromise;
  }

  private async initializeWallet(seed?: string): Promise<WalletWithPrivateKey> {
    let wallet = await this.walletManager.loadWallet();

    if (!wallet) {
      console.log(chalk.yellow("No wallet found. Creating new wallet..."));
      wallet = await this.walletManager.createWallet(seed);
