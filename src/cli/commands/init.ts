...
export const initCommand = new Command("init").description("Initialize x402 in existing project").action(async () => {
  const spinner = ora();
  ...
  try {
    ...
    const updatedLines = envLines.filter((line) => !line.startsWith("X402_PRIVATE_KEY=") && !line.startsWith("X402_WALLET_ADDRESS="));
    ...
    const hasX402 = gitignoreLines.some((line) => line.trim() === ".x402/");
    const hasEnv = gitignoreLines.some((line) => line.trim() === ".env");
